...
import {
  ...
  FileText, Inbox, LayoutDashboard, ListChecks, Mail, Plus, RefreshCw, Save, Search,
...
function App() {
  ...
  const load=async()=>{
    const [dash,apps,opps,profile,emailMatches,appSettings]=await Promise.all([
      api<Dashboard>("/dashboard"),api<Application[]>("/applications"),api<Opportunity[]>("/opportunities"),
      api<Fact[]>("/profile"),api<EmailMatch[]>("/email-matches"),api<AppSettings>("/settings"),
    ]);
    setDashboard(dash);setApplications(apps);setOpportunities(opps);setFacts(profile);setMatches(emailMatches);setSettings(appSettings);
  };
